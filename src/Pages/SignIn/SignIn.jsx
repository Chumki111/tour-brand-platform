...
const SignIn = () => {
    ...
    return (
      ...
      {/* img */}
      <div className={`absolute w-1/2 h-full top-0 z-50 min-h-full duration-500 overflow-hidden hidden lg:block  bg-sky-500 `}><img src="https://source.unsplash.com/random" className="object-cover h-full" alt="img"/></div>
      {/* login form */}
      <form  onSubmit={handleRegister} className={`p-8 w-full lg:w-1/2 mr-0 ml-auto duration-500`}>
        <h1 className="backdrop-blur-sm text-4xl pb-4">Login</h1>
        <div className="space-y-5">
          <label htmlFor="email" className="block">Email</label>
          <input id="email" name="email" type="email" placeholder="example@example.com" className="p-3 block w-full drop-shadow-lg outline-none border rounded-md invalid:border-red-700 valid:border-black"/>
          <label htmlFor="password"  className="block">Password</label>
          <input id="password" name="password" type="password" placeholder=".............." min={5} className="p-3 block w-full drop-shadow-lg outline-none border rounded-md invalid:border-red-700 valid:border-black"/>
        </div>
        ...
        <p className="mb-3 text-center">Don't have an account?<Link className="underline">Register</Link></p>
        <hr />
